...
- 可设置月流量额度、查看月底预测；
- 可在面板中给账号设置备注名；
- 使用可撤销的浏览器会话，可选择记住设备 30 天并管理已登录设备；
- 查看有数量上限的登录、配置、规则操作和采集状态审计日志；
...
密码和会话令牌都只以安全哈希保存在 `traffic.db` 中。默认登录只持续到
当前浏览器会话结束；勾选“记住此设备”后有效期为 30 天。“设置”页面可以
撤销单个设备、退出所有设备或修改密码；修改密码会让其他会话全部失效。
...
正常时侧栏会显示“采集正常”、运行时长、最近更新时间和项目版本。首次启动只能从程序创建计数规则以后开始记录，无法补回此前的历史流量。
...
```text
./config.yaml        # 非敏感运行设置
./data/traffic.db    # 流量、规则、密码哈希和登录会话
./data/log.db        # 登录、操作和采集状态审计日志
```
...
并保存在 `./config.yaml`。统计时区默认为 UTC，需要按其他地区自然日聚合时
可以修改。网站明细采集固定启用。
collector 会自动读取更新后的配置。认证数据仍只保存在 `traffic.db`。
审计日志使用独立的 SQLite WAL 数据库 `data/log.db`，以 UTC 保存、按浏览器
本地时区显示，最多保留 5,000 条；“日志”页面支持 UTC 日期筛选、关键词
搜索、自动刷新和清理。首次从合并数据库版本升级时，程序会先把旧审计日志
复制到 `log.db`，成功后再从 `traffic.db` 删除旧日志表。更早版本保存在数据库
中的运行配置也会自动迁移到 YAML，迁移成功后删除旧配置表。

网站明细默认保留 180 天，按“日期 + 设备 + 域名”聚合，不保存 URL、
请求路径或单条连接。

用户和设备的访问记录默认显示“过去 1 天”（滚动 24 小时），不受统计时区
影响，也可切换为按日期查看。短期网站明细按 15 分钟聚合并仅保留约 25 小时；
访问时间由浏览器按查看者本地时区显示。升级后短期明细从零开始积累。
...
用户和设备的下载、上传、总计均以“过去 1 天/所选月份”显示。过去 1 天
采用 5 分钟聚合并仅保留约 25 小时，不保存连接明细；升级后该统计
从零开始积累，当月累计不受影响。
